...
Numerical simulations are widely used in science and engineering to study complex physical phenomena and optimize system designs. A typical simulation workflow involves several key steps: computational geometry creation or import, mesh generation, physical model setup, numerical solution, and post-processing the results for analysis. Each of these steps requires specialized tools and expertise. While powerful specialized tools exist for individual steps, integrating them into a seamless, automated pipeline remains challenging. Data exchange issues, inconsistent user interfaces, and limited scripting capabilities often lead to time-consuming manual work and reduced reproducibility. SALOME was developed to address this fragmentation by offering a unified, extensible open-source platform, hosting all these services under one roof.

# State of the Field

For numerical simulations, commercial platforms such as ANSYS, Abaqus, COMSOL Multiphysics,  offer integrated environments but are constrained by high licensing costs and limited customizability. In the open-source community, powerful individual tools have emerged, such as FreeCAD for parametric CAD, Gmsh and Netgen for meshing, ParaView for visualization, and solvers such as OpenFOAM, MOOSE, and FreeFEM. These components are powerful and widely adopted, yet combining them into automated workflows remains cumbersome due to interoperability and interface issues. SALOME addresses this gap and provides a tightly integrated open-source alternative. The individual modules of SALOME are described in detail in the following section.

# Software Design

SALOME is built on a modular architecture offering a comprehensive suite of tools for the entire simulation workflow.

This modularity is a deliberate design choice in SALOME over a monolithic, single-purpose tool. Each module (CAD, meshing, visualization, etc.) is developed and maintained largely independently, each in its own repository. This makes it easier to add, update, remove, or replace individual capabilities of a module over time, but it introduces an integration challange: modules must interoperate reliably with one another despite being developed on separate codebases. SALOME addresses this integration challange through two mechanisms. The SAT build tool compiles and packages the independently developed modules into a single consistent platform, exposing them together under one GUI regardless of how each module is built or versioned internally. At the scripting level, a common Python API lets objects (geometries, meshes, fields) created in one module be passed directly to another, so a workflow can move from CAD to meshing to visualization without manual data conversion between modules. This design matters because it allows the same core platform to be reconfigured into domain-specific applications (e.g., SALOME_CFD [@salomecfd], SALOME_MECA [@AsterAster]) simply by recomposing existing modules, rather than rebuilding integration logic from scratch. The individual modules that make up this architecture are described in detail below.
